...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<div class="row">
		...
		<?php echo $form->dropDownList($model, 'template_id', CHtml::listData(InvoicesFktTmpl::model()->findAll(),'id','name')); ?>
	</div>
	...
	<div class="row">
		<?php echo $form->labelEx($model,'cargo_addr'); ?>
...
function change_order()
	{
		var code = {
			'order_id' : $('select[name=\"InvoicesFkt[order_id]\"] option:selected').val(),
		}
		...
		$.ajax({
			...
			//dataType: 'JSON',
			data: code,
			url: '" . $this->createUrl('InvoicesFkt/changeorder') . "',
			...
			{//alert(msg);
				if (msg!='null')
				{
					...
					$(\"select[name='InvoicesFkt[act_id]'] option\").remove();
					$.each(eval(msg),function(i,item)	{
					$(\"select[name='InvoicesFkt[act_id]']\").append('<option value=\"'+item.optionKey+'\">'+item.optionValue+'</option>');
					});
					//$(\"select[name='InvoicesFkt[act_id]']\").html(options);
...
function change_act()
	{
		$('#ajax_mess_act').text('');
		$('#InvoicesFkt_sum').val('');
		$('select[name=\"InvoicesFkt[act_id]\"] option:selected').each(function()
		{
			var code = {
				'act_id' : $(this).val(),
			}
			...
			$.ajax({
				...
				//dataType: 'JSON',
				data: code,
				url: '" . $this->createUrl('InvoicesFkt/changeact') . "',
