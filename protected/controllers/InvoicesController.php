...
//		$this->redirect('admin');//Yii::app()->request->urlReferrer);
	...
	public function actionUpdate($id) {
		...
		if (isset($_POST['Invoices'])) {
			$model->attributes = $_POST['Invoices'];
			if ($model->save()) {
				$msg = 'Счёт #' . $model->id . ' для Заказа #' . $model->order_id . ' ' . $model->order->name . ' изменён';
				Yii::app()->user->setFlash('success', $msg);
				Yii::app()->logger->write($msg);
				$this->redirect(array('view', 'id' => $model->id));
			}
	...
	public function actionDelete($id) {
		if (Yii::app()->request->isPostRequest) {
			...
			$model = $this->loadModel($id);
			$c = Payments::model()->countByAttributes();
			if (null == $c) {
				if ($model->delete()) {
					$msg = 'Счёт #' . $model->id . ' для Заказа #' . $model->order_id . ' ' . $model->order->name . ' удалён';
					Yii::app()->user->setFlash('success', $msg);
					Yii::app()->logger->write($msg);
				}
			}
			else {
				$msg = 'Счёт #' . $model->id . ' для Заказа #' . $model->order_id . ' ' . $model->order->name . ' нельзя удалить. Есть платежи: ' . $c;
...
//		Dumper::d($_POST['order_id']);die;
	...
	public function actionHtml($id) {
		...
		$data['invoice'] = $this->loadModel($id);
		if (!is_array($works_pos = Works::model()->worksByOrder($data['invoice']->order_id, $data['invoice']->act_id))) {
			Yii::app()->user->setFlash('error', 'Список работ для счёта #' . $data['invoice']->id . ' пуст');
			$this->redirect(array('view', 'id' => $data['invoice']->id));
		}
		else
			$data['works'] = $works_pos;
		if (is_null($data['client'] = Clients::model()->findByPk($data['invoice']->client_id))) {
			Yii::app()->user->setFlash('error', 'Данные клиента для счёта #' . $data['invoice']->id . ' пусты');
			$this->redirect(array('view', 'id' => $data['invoice']->id));
		}
		if (!is_array($data['settings'] = Config::model()->get_settings())) {
			Yii::app()->user->setFlash('error', 'Основные параметры для счёта #' . $data['invoice']->id . ' пусты');
			$this->redirect(array('view', 'id' => $data['invoice']->id));
		}
		$data['body'] = $this->_tmpl_body($data, InvoicesTmpl::model()->findByPk($data['invoice']->template_id));
		$this->layout = false;
	...
	private function _tmpl_body($data, $tmpl) {
		...
		$map = array(
			//'{name}' => $data['act']->name,
			'inv_sum_num' => $data['invoice']['sum'],
			'inv_sum' => $numstr->convert($data['invoice']['sum']),
			'inv_date' => $data['invoice']['date'] ? (Yii::app()->dateFormatter->format('d MMMM yyyy', $data['invoice']['date'], TRUE)) : ('"___" _________ 20 __ г.'),
			'inv_num' => $data['invoice']['num'],
			'works' => $data['works'],
